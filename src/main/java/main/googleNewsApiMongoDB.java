...
public class googleNewsApiMongoDB {
	...
	public static final String[] NED = { "uk", "au", "in", "en_il", "en_my", "nz", "en_pk", "en_ph", "en_sg", "ar_me",
			"ar_ae", "ar_lb", "ar_sa", "cn", "hk", "hi_in", "ta_in", "ml_in", "te_in", "iw_il", "jp", "kr", "tw",
			"vi_vn", "nl_be", "fr_be", "en_bw", "cs_cz", "de", "es", "en_et", "fr", "en_gh", "en_ie", "it", "en_ke",
			"hu_hu", "fr_ma", "en_na", "nl_nl", "en_ng", "no_no", "de_at", "pl_pl", "pt-PT_pt", "de_ch", "fr_sn",
			"en_za", "fr_ch", "sv_se", "en_tz", "tr_tr", "en_ug", "en_zw", "ar_eg", "el_gr", "ru_ru", "sr_rs", "ru_ua",
			"uk_ua", "es_ar", "pt-BR_br", "ca", "fr_ca", "es_cl", "es_co", "es_cu", "es_us", "es_mx", "es_pe", "us",
			"es_ve" };
	...
	public static final int wholeProcessIsRepeated = 3; // In hours
		...
		try {
			...
			while (true) {
				...
				for (int l = 0; l < NED.length; l++) {
					String urlWithCountryParam = urlWithIntialParams + "&ned=" + NED[l];
							...
							while (true) {
								if (counterToExitLoop > 5) {
									...
									if (GlobalErrorCounter < 5) {
										...
										// System.out.println("chainging the
										// topic!");
										// break topicLoop;
									} else {
								...
								int randomWait = rn.nextInt((maxWait - minWait) + 1) + minWait;
								...
								if (responseStatus == 200) {
									...
									GlobalErrorCounter = 0;
									logger.info("Processed " + NED[l] + "_" + TOPICSDes[i] + "_" + ((j / 8) + 1) + "_"
											+ ".json");
									...
									logger.info("403 error is encountered with " + NED[l] + "_" + TOPICSDes[i] + "_"
											+ ((j / 8) + 1));
									counterToExitLoop++;
									Thread.sleep(300000);
	...
	public static void insertArticleIntoMongoDB(Article article, String topic, String ned) {
